...
abund_septData5 <- septData5 %>% 
  filter(General.Reef.Type.x == "Designed reefs" | General.Reef.Type.x == "Rubble and rocks")
...
latModelAbund[[3]] <- lmer(abundance.CohenD ~ LatZone + (1|ID), data = abund_septData5)
latModelAbund[[4]] <- lmer((abundance.CohenD) ~ Ocean.Basin.x + LatZone + (1|ID), data = abund_septData5)
...
biomassModelData <- subset(septData5, c(septData5$LatZone != "subtropical" & septData5$LatZone != "subarctic" & (septData5$General.Reef.Type.x == "Designed reefs" | septData5$General.Reef.Type.x == "Rubble and rocks")))
latModelBio2 <- list()
...
ecoModelBio[[4]] <- lmer(biomass.CohenD ~ Openings.x + (1|ID), data = biomassData_model)
##### Double factor
ecoModelBio[[5]] <- lmer(biomass.CohenD ~ Rugosity.x + Vertical.relief.x + (1|ID), data = biomassData_model)
ecoModelBio[[6]] <- lmer(biomass.CohenD ~ Rugosity.x + Openings.x + (1|ID), data = biomassData_model)
ecoModelBio[[7]] <- lmer(biomass.CohenD ~ Vertical.relief.x + Openings.x + (1|ID), data = biomassData_model)
##### Triple factor
ecoModelBio[[8]] <- lmer(biomass.CohenD ~ Rugosity.x + Vertical.relief.x + Openings.x + (1|ID), data = biomassData_model)
...
row1 <- plot_grid(Newpanel1 , Newpanel2, align = "h", labels = c("A", "B"),
                  label_x = c(-0.005,-0.05), label_size = 24)
...
row2 <- plot_grid(Newpanel6, Newpanel7, align = "h", nrow = 1, labels = c("C", "D"), label_x = -0.005, label_size = 24)
